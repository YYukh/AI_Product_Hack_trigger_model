...
def test_immediate_delivery_does_not_require_a_fresh_moex_candle():
    signal = pd.DataFrame([{
        "event_id": "event-1", "available_at": pd.Timestamp("2026-01-10"),
        "currency": "KZT", "scenario": "GOOD_NOW", "target_family": "G0",
        "target": "target-g0", "horizon": 3, "expected_bps": 100.0,
    ...
    immediate = details.loc[details["delivery_delay_hours"].eq(0)]
    delayed = details.loc[details["delivery_delay_hours"].gt(0)]
    assert len(immediate) == 6
    assert immediate["is_relevant"].all()
    assert set(immediate["relevance_status"]) == {"DIRECT_DELIVERY"}
    assert not delayed["is_relevant"].any()
    assert len(delayed) == 4
    assert set(delayed["relevance_status"]) == {"STALE_QUOTE"}
